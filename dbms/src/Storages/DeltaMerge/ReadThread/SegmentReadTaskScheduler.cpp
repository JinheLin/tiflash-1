    ...
    std::unique_lock<std::mutex> & lock)
{
    ...
    auto segment = scheduleSegment(pool, lock);
    if (!segment)
    {
    ...
    auto pools = getPools(segment->second, lock);
    if (pools.empty())
    {
...
SegmentReadTaskPools SegmentReadTaskScheduler::getPools(
    const std::vector<uint64_t> & pool_ids,
    std::unique_lock<std::mutex> & /*lock*/)
{
...
std::optional<std::pair<GlobalSegmentID, std::vector<UInt64>>> SegmentReadTaskScheduler::scheduleSegment(
...
bool SegmentReadTaskScheduler::schedule()
{
    ...
    if (read_pools.empty())
    {
        cv.wait(lock, [&]() { return !read_pools.empty() || isStop(); });
    }
    if (isStop())
    {
        return true; // not sleep
    }
    ...
    for (auto itr = read_pools.begin(); itr != read_pools.end(); /**/)
    {
        ...
        auto merged_task = scheduleMergedTask(pool, lock);
        if (merged_task == nullptr)
        {
            ++sched_null_count;
            continue;
