...
bool DMFileReader::getSkippedRows(size_t & skip_rows)
{
    ...
    for (; next_pack_id < pack_res.size() && pack_res[next_pack_id] == RSResult::None; ++next_pack_id)
    {
...
size_t DMFileReader::getReadRows()
{
    ...
    const auto & pack_stats = dmfile->getPackStats();
    size_t read_rows = 0;
    for (; next_pack_id < pack_res.size() && pack_res[next_pack_id] != RSResult::None && read_rows < rows_threshold_per_read;
         ++next_pack_id)
    {
