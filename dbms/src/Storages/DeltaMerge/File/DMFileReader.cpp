...
bool DMFileReader::getSkippedRows(size_t & skip_rows)
{
    ...
    for (; next_pack_id < pack_res.size() && !pack_res[next_pack_id].needToRead(); ++next_pack_id)
    {
...
size_t DMFileReader::getReadRows()
{
    ...
    const auto & pack_stats = dmfile->getPackStats();
    size_t read_rows = 0;
    for (; next_pack_id < pack_res.size() && pack_res[next_pack_id].needToRead() && read_rows < rows_threshold_per_read;
         ++next_pack_id)
    {
...
Block DMFileReader::readWithFilter(const IColumn::Filter & filter)
{
    ...
    for (size_t pack_id = start_pack_id; pack_id < last_pack_id; ++pack_id)
    {
        ...
        if (pack_res[pack_id].needToRead() && (pack_id + 1 == pack_res.size() || !pack_res[pack_id + 1].needToRead()))
        {
            Block block = read();
        ...
        else if (!pack_res[pack_id].needToRead())
        {
...
Block DMFileReader::read()
{
    ...
    if (is_fast_scan)
    {
        ...
        if (enable_handle_clean_read)
        {
            ...
            for (size_t i = start_pack_id; i < next_pack_id; ++i)
            {
                ...
                if (handle_res[i] == RSResultConst::All
                ...
                else if (!enable_del_clean_read && handle_res[i] == RSResultConst::All)
                {
                    handle_column_clean_read_packs.push_back(i);
                }
            }
    ...
    else if (enable_handle_clean_read)
    {
        ...
        for (size_t i = start_pack_id; i < next_pack_id; ++i)
        {
            ...
            if (handle_res[i] == RSResultConst::All && pack_stats[i].not_clean == 0
