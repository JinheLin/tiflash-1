...
PushDownFilterPtr PushDownFilter::build(
    ...
    const LoggerPtr & tracing_logger)
{
    auto lm_filter = QueryFilter::build(table_scan_column_info, lm_filters, columns_to_read, context, tracing_logger);
    auto ordinary_filter
        = QueryFilter::build(table_scan_column_info, ordinary_filters, columns_to_read, context, tracing_logger);
    return std::make_shared<PushDownFilter>(rs_operator, lm_filter, ordinary_filter);
}
