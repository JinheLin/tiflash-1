...
String SegmentReadTask::info() const
{
    return extra_remote_info.has_value() ? extra_remote_info->remote_segment_id.toString() : "";
}
...
BlockInputStreamPtr SegmentReadTask::getInputStream() const
{
    RUNTIME_CHECK(input_stream != nullptr);
    return input_stream;
}
...
{
    const auto & extra_info = *extra_remote_info;
    ...
    const std::vector<Remote::PageOID> & pages_not_in_cache) const
{
    ...
    auto keyspace_id = extra_remote_info->remote_segment_id.keyspace_id;
    ...
    *req.mutable_snapshot_id() = meta;
    req.set_table_id(extra_remote_info->remote_segment_id.physical_table_id);
    req.set_segment_id(extra_remote_info->remote_segment_id.segment_id);
        ...
        for (const String & page : packet->pages())
        {
            if (write_page_task == nullptr)
            {
                write_page_task = std::make_unique<WritePageTask>(
                    extra_remote_info->dm_context->db_context.getSharedContextDisagg()->rn_page_cache.get());
            }
            ...
            RUNTIME_CHECK_MSG(parsed, "Failed to parse page data (from {})", info());
            ...
            auto oid = Remote::PageOID{
                .store_id = extra_remote_info->remote_segment_id.store_id,
                .ks_table_id
                = {extra_remote_info->remote_segment_id.keyspace_id,
                   extra_remote_info->remote_segment_id.physical_table_id},
            ...
            auto write_batch_limit_size
                = extra_remote_info->dm_context->db_context.getSettingsRef().dt_write_page_cache_limit_size;
