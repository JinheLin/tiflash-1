...
public:
    ...
    bool operator!=(basic_interval_tree_iterator const & other) const { return node_ != other.node_; }
    ...
    /**
         ...
         */
    typename node_type::interval_type::value_type max() const { return node_->max(); }
    ...
    typename tree_type::interval_type interval() const { return node_->interval(); }
...
public:
    ...
    template <typename CompareFunctionT>
    const_iterator find_next_in_subtree(iterator from, interval_type const & ival, CompareFunctionT const & compare)
        const
    {
        if (root_ == nullptr)
            return end();
    ...
    const_iterator overlap_find_next_in_subtree(const_iterator from, interval_type const & ival, bool exclusive = false)
        const
    {
        if (root_ == nullptr)
            return end();
...
private:
    ...
    iterator insert_merge_set(interval_type const & interval, bool) { return insert(interval); }
