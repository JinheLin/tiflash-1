...
protected:
    ...
    {
        DB::FailPointHelper::disableFailPoint(DB::FailPoints::delta_tree_create_node_fail);
        std::cout << fmt::format("TearDown: fake_tree={}\n", fake_tree->getBytes());
    }
...
try
{
    ...
    std::cout << "mem_usage_old: " << mem_usage_old << std::endl;
    std::cout << "tree.getBytes(): " << tree.getBytes() << std::endl;
