    ...
    const LoggerPtr & tracing_logger)
{
    ...
    if (context.getSettingsRef().filter_execution_mode == FilterExecutionMode::LM && !dag_query->filters.empty())
    {
    ...
    return PushDownFilter::build(
        rs_operator,
        columns_to_read_info,
        /*lm_filter*/ pushed_down_filters,
        /*ordinary_filter*/ context.getSettingsRef().filter_execution_mode == FilterExecutionMode::Storage
            ? dag_query->filters
