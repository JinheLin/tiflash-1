...
TiFlash C++ 侧维护 3 个进程内存值：

- `raw_rss`：原始进程 RSS。
- `rss_file`：`/proc/self/status` 中的 `RssFile`。
- `memory_control_rss`：实际用于内存流控的 RSS。
...
- 在 `CollectProcInfoBackgroundTask::memCheckJob()` 中定期更新这些值，更新频率沿用当前 `MemTrackThread` 的 100 ms 周期。
- `MemoryTracker` 继续读取现有全局流控 RSS 变量，但该变量改为保存 `memory_control_rss`。
